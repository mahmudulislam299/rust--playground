...
impl Price for CoinPrice 
{
	fn print_coinprice(&self) -> String 
	{
		return format!("SPOT: {base}-{currency}: {amount}", base=self.base,currency=self.currency, amount=self.amount);
	}
	fn format_coinamount(&self) -> String 
	{
...
{    
	...
	let resp_coin_time = client.get("https://api.coinbase.com/v2/time").send();

	match resp_coin_time 
	{
		Ok(parsed_coin_time) => 
		{
			let coinbasetime = parsed_coin_time.json::<CoinbaseTime>().unwrap();
